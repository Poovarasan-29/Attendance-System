...
import { useGetAllAttendanceQuery, useGetDepartmentsQuery } from '../store/apiSlice';
...
const Reports = () => {
    ...
    const filteredData = allAttendance.filter(item => {
        let matches = true;

        if (filters.startDate && filters.endDate) {
            if (item.date < filters.startDate || item.date > filters.endDate) matches = false;
        }

        if (filters.employeeId) {
            const search = filters.employeeId.toLowerCase();
            const empId = item.userId?.employeeId?.toLowerCase() || '';
            const name = item.userId?.name?.toLowerCase() || '';
            if (!empId.includes(search) && !name.includes(search)) matches = false;
        }

        if (filters.department) {
            if (item.userId?.department?.toLowerCase() !== filters.department.toLowerCase()) matches = false;
        }
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-purple-500"
                                >
                                    <option value="" className="text-slate-500">All Departments</option>
