...
import { useGetAllEmployeesQuery, useGetDepartmentsQuery } from '../store/apiSlice';

const AllEmployees = () => {
    const { user } = useSelector((state) => state.auth);

    // RTK Query Hooks
    const { data: employees = [], isLoading: isEmployeesLoading } = useGetAllEmployeesQuery(undefined, { refetchOnMountOrArgChange: true });
    ...
    const filteredEmployees = employees.filter(emp => {
        ...
        if (appliedFilter.startDate || appliedFilter.endDate) {
            ...
            const empDate = toYMD(emp.createdAt); // Changed from doj to createdAt as per User model
